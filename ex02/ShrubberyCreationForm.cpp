...
/*   Updated: 2025/12/05 13:43:03 by aaleixo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ShrubberyCreationForm::execute(Bureaucrat a)
{
    if (this->getSigned() == true && a.getGrade() <= this->getExecGrade())
    {
        std::ofstream shrubFile(this->_target.append("_shrubbery").c_str());
        shrubFile << "                                                 ." << std::endl;
        shrubFile << "                                      .         ;  " << std::endl;
        shrubFile << "         .              .              ;%     ;;   " << std::endl;
        shrubFile << "           ,           ,                :;%  %;   " << std::endl;
        ...
        shrubFile << "                  ...;%@@@@@%%:;;;;,.." << std::endl;
        shrubFile.close();
        std::cout << a.getName() << " created Shubbery " << this->getName() << " in the working directory with the name " << this->getName().append("_shrubbery") << std::endl;
    }
    else if (a.getGrade() > this->getExecGrade())
        std::cout << a.getName() << "'s grade too low to execute a shrubbery creation..." << std::endl;
    else
        std::cout << "Shrubbery creation form not signed yet" << std::endl;
}